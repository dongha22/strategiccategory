    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-8">
        ...
        <table className="w-full text-left text-sm border-collapse relative">
          <thead className="text-slate-500 font-medium sticky top-0 z-10 shadow-sm">
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th rowSpan={2} className="px-4 py-3 w-28 text-right border-r border-slate-200 bg-slate-50 whitespace-nowrap">25년 실적</th>
              <th rowSpan={2} className="px-4 py-3 w-28 text-right border-r border-slate-200 bg-slate-50 whitespace-nowrap">26년 실적</th>
              ...
              <th rowSpan={2} className="px-6 py-3 w-24 text-center border-l border-slate-200 bg-slate-50">상태</th>
            </tr>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-2 py-2 text-center text-[11px] w-[100px] border-r border-slate-200 bg-slate-50">25</th>
              <th className="px-2 py-2 text-center text-[11px] w-[100px] border-r border-slate-200 bg-slate-50">26 Q1</th>
              <th className="px-2 py-2 text-center text-[11px] w-[100px] border-r border-slate-200 bg-slate-50">26 Q2</th>
              <th className="px-2 py-2 text-center text-[11px] w-[100px] border-r border-slate-200 bg-slate-50">26 Q3</th>
              <th className="px-2 py-2 text-center text-[11px] w-[100px] bg-slate-50">26 Q4</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 bg-white">
            {filteredCustomers.map((c, idx) => (
              <tr key={c.id} className="hover:bg-slate-50/80 transition-colors group">
                ...
                <td 
                  className="px-4 py-4 font-bold text-blue-600 whitespace-nowrap border-r border-slate-100/50 cursor-pointer hover:text-blue-800 hover:underline transition-colors"
                  onClick={() => onCustomerClick(c)}
                >
                  {c.name}
                </td>
                <td className="px-4 py-4 text-right font-medium text-slate-500 whitespace-nowrap border-r border-slate-100/50">{c.revenueLastYear}억</td>
                ...
                <td className="px-4 py-4 text-right border-r border-slate-100/50">
                  {c.revenueLastYear === 0 ? (
                    <span className="inline-flex items-center font-semibold text-xs text-blue-600">new</span>
                  ) : (
                ...
                {c.shares.map((s, sIdx) => {
                  const isLastYear = s.period === '25';
                  const isEmpty = s.cosmax === 0 && s.kolmar === 0 && s.others === 0;
                  return (
                    <td key={sIdx} className={`px-3 py-4 ${sIdx !== 4 ? 'border-r border-slate-100/30' : ''}`}>
                      {isEmpty ? (
                        <div className="flex items-center justify-center h-[30px] text-slate-300 text-xs">-</div>
                      ) : (
      ...
      <div className="bg-slate-50 px-6 py-3 border-t border-slate-100 flex items-center gap-6 relative z-10">
        ...
        <div className="ml-auto">
           <span className="text-[9px] text-slate-400 italic">※ 흐린 막대는 25년 베이스 데이터입니다. 스크롤하여 전체 데이터를 확인하세요.</span>
        </div>
      </div>
    </div>
  );
